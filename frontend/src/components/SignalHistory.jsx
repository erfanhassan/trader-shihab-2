...
const SignalHistory = ({ history, marketData, onClearHistory }) => {
  ...
  if (!history || history.length === 0) {
    return (
      <div className="bg-white rounded-xl border border-slate-300 p-6 shadow-lg h-full flex flex-col justify-center items-center text-slate-500">
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-300 shadow-lg flex flex-col h-full max-h-[400px]">
      <div className="p-4 border-b border-slate-300 flex items-center justify-between bg-white/50 rounded-t-xl">
        ...
        <div className="flex items-center gap-3">
          <select 
            ...
            className="text-xs bg-slate-50 border border-slate-300 text-slate-700 rounded-md px-2 py-1 outline-none"
          >
          ...
          <div className="text-xs text-slate-500 font-medium px-2 py-1 bg-slate-50 rounded-md">
            {filteredHistory.length} signals
          </div>
          {history.length > 0 && (
            <button 
              onClick={onClearHistory}
              className="text-xs text-rose-600 hover:text-rose-300 bg-rose-500/10 hover:bg-rose-500/20 px-2 py-1 rounded-md flex items-center gap-1 transition-colors"
      ...
      <div className="overflow-y-auto p-4 flex-1 space-y-3">
        {sortedHistory.map((trade) => {
          ...
          return (
            <div key={trade.id} className="bg-slate-50/50 rounded-lg p-3 border border-slate-300/50 hover:border-slate-600 transition-colors">
              <div className="flex justify-between items-start mb-2">
                <div className="flex items-center gap-2">
                  <span className={`font-bold ${trade.direction === 'LONG' ? 'text-emerald-600' : 'text-rose-600'}`}>
                    {trade.direction}
                  </span>
                  <span className="text-slate-700 font-medium">{trade.symbol}</span>
                  <span className="text-[10px] px-2 py-0.5 rounded-full bg-slate-50 text-slate-500 border border-slate-300">
              ...
              <div className="grid grid-cols-3 gap-2 text-xs text-slate-500 mt-2">
                ...
                <div>
                  ...
                  <span className="font-mono text-rose-600/80">{trade.sl?.toFixed(2)}</span>
                </div>
                <div>
                  <span className="block text-slate-500 mb-0.5">TP</span>
                  <span className="font-mono text-emerald-600/80">{trade.tp?.toFixed(2)}</span>
                </div>
              </div>
