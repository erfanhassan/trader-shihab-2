...
export default function SignalBox({ signal }) {
  ...
  if (!signal) {
    return (
      <div className="bg-white p-8 rounded-xl border border-slate-300 flex flex-col items-center justify-center min-h-[300px] text-center transition-all duration-500">
  ...
  const textColor = isLong ? 'text-emerald-600' : 'text-rose-600';
    ...
    <div className={`p-4 md:p-8 rounded-xl border-2 ${bgColor} ${borderColor} shadow-[0_0_50px_rgba(0,0,0,0.3)] shadow-${isLong?'emerald':'rose'}-500/20 transition-all duration-300 min-h-[300px] flex flex-col justify-between animate-pulse`}>
      <div className="flex flex-col sm:flex-row sm:justify-between items-start gap-4">
        <div>
          ...
          <p className="text-slate-700 text-base md:text-xl font-medium">{signal.symbol} • 1m Timeframe</p>
        </div>
        <div className="text-left sm:text-right">
          <p className="text-slate-500 text-xs md:text-sm uppercase tracking-wider">Timestamp</p>
          <p className="text-slate-800 text-sm md:text-base font-mono">{new Date(signal.timestamp).toLocaleTimeString()}</p>
        </div>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 md:gap-6 mt-6 md:mt-8">
        <div className="bg-slate-50/60 p-3 md:p-4 rounded-lg border border-slate-300/50 min-w-0">
          <div className="flex items-center gap-2 text-slate-500 mb-1 md:mb-2 text-sm md:text-base">
        ...
        <div className="bg-slate-50/60 p-3 md:p-4 rounded-lg border border-rose-900/50 min-w-0">
          <div className="flex items-center gap-2 text-rose-600 mb-1 md:mb-2 text-sm md:text-base">
            <Shield size={16} className="md:w-[18px] md:h-[18px]" /> <span>Stop Loss</span>
          </div>
          <div className="text-lg md:text-xl lg:text-2xl font-mono font-bold text-rose-600 break-all" title={signal.sl}>{Number(signal.sl).toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 6})}</div>
        </div>
        <div className="bg-slate-50/60 p-3 md:p-4 rounded-lg border border-emerald-900/50 min-w-0">
          <div className="flex items-center gap-2 text-emerald-600 mb-1 md:mb-2 text-sm md:text-base">
            <Target size={16} className="md:w-[18px] md:h-[18px]" /> <span>Take Profit</span>
          </div>
          <div className="text-lg md:text-xl lg:text-2xl font-mono font-bold text-emerald-600 break-all" title={signal.tp}>{Number(signal.tp).toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 6})}</div>
        </div>
      </div>
    </div>
  );
